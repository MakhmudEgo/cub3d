...
{
	short x_u = (data->strt_x + cos(t_c3d.crnr) * 4);
	short y_u = (data->strt_y + sin(t_c3d.crnr) * 4);
	short x_d = (data->strt_x - cos(t_c3d.crnr) * 4);
	short y_d = (data->strt_y - sin(t_c3d.crnr) * 4);
	if (UP)
	{
		if ((t_c3d.map)[y_u / SZ_PX][x_u / SZ_PX] != '1')
		{
			data->strt_x += cos(t_c3d.crnr) * 4;
			data->strt_y += sin(t_c3d.crnr) * 4;
	...
	if (LEFT)
	{
		if ((t_c3d.map)[(int)(data->strt_y - cos(t_c3d.crnr) * 4)/ SZ_PX][(int)(data->strt_x + sin(t_c3d.crnr) * 4) / SZ_PX] != '1')
		{
			data->strt_x += sin(t_c3d.crnr) * 4;
			data->strt_y -= cos(t_c3d.crnr) * 4;
			t_c3d.crnr_s = t_c3d.crnr - M_PI_6;
			parse_map(data);
		}
	}
